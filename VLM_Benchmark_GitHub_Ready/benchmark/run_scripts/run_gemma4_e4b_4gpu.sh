#!/usr/bin/env bash
# run_gemma4_e4b_4gpu.sh
# Full pipeline for Gemma4-E4B on 800-image benchmark (4-GPU variant):
#   Step 1 — shard annotations (4 shards)
#   Step 2 — run benchmark (4 shards x 1 GPU in parallel)
...
NUM_SHARDS=4
# GPU 0 has only ~5GB free so 4bit is needed; use for all shards for consistency
USE_4BIT="${USE_4BIT:-1}"
RUN_DIR="$RESULTS_BASE/${MODEL_NAME}_800_4gpu_${RUN_ID}"
...
# ─── Step 1: split annotation into 4 shards ──────────────────────────────────
log "=== Step 1: Preparing annotation shards ($NUM_SHARDS shards) ==="
$PYTHON - <<PY "$ANN_800" "$SHARD_DIR" "$NUM_SHARDS"
import json, os, sys
ann_path, shard_dir, n = sys.argv[1], sys.argv[2], int(sys.argv[3])
...
for i in range(n):
    d = dict(ann)
    d['items'] = [it for idx, it in enumerate(items) if idx % n == i]
    with open(os.path.join(shard_dir, f'ann_shard_{i}.json'), 'w', encoding='utf-8') as f:
        json.dump(d, f, ensure_ascii=False, indent=2)
print(f"Created {n} shards from {len(items)} items ({len(items)*5} expected rows total)")
PY

# ─── Step 2: run 4 shards in parallel (1 GPU each) ───────────────────────────
log "=== Step 2: Launching $NUM_SHARDS shards in parallel ==="
PIDS=()
...
cp -f "$FINAL_CSV" \
    "$FINAL_RESULTS_DIR/${MODEL_NAME}_800_4gpu_${RUN_ID}__results_merged_800.csv"
...
cp -f "$ASR_CSV"     "$FINAL_RESULTS_DIR/${MODEL_NAME}_800_4gpu_${RUN_ID}__results_merged_800__asr_eval_0106.csv"     2>/dev/null || true
cp -f "$ASR_SUMMARY" "$FINAL_RESULTS_DIR/${MODEL_NAME}_800_4gpu_${RUN_ID}__results_merged_800__asr_summary_0106.csv" 2>/dev/null || true
...
if [[ -z "${OPENAI_API_KEY:-}" ]]; then
    ...
    cp -f "$SCORED_CSV"    "$FINAL_RESULTS_DIR/${MODEL_NAME}_800_4gpu_${RUN_ID}__results_scored_pure_gpt.csv"  2>/dev/null || true
    cp -f "$TONE_SUMMARY"  "$FINAL_RESULTS_DIR/${MODEL_NAME}_800_4gpu_${RUN_ID}__tone_summary_pure_gpt.csv"   2>/dev/null || true
    cp -f "$SPLIT_SUMMARY" "$FINAL_RESULTS_DIR/${MODEL_NAME}_800_4gpu_${RUN_ID}__split_summary_pure_gpt.csv"  2>/dev/null || true
fi
